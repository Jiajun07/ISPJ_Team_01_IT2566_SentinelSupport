...
class ComplianceService:
    ...
    def generateComplianceData(tenantID=None, startDate=None, endDate=None):
        if not startDate:
            startDate = datetime.now(SGT) - timedelta(days=30)
        if not endDate:
            endDate = datetime.now(SGT)
    ...
    def exportCompliancePDF(logs, summary, start_date, end_date, tenant_name, generated_by):
        ...
        try:
            ...
            report_info_data = [
                ...
                ['Generated On:', datetime.now(SGT).strftime('%d %b %Y at %H:%M')],
                ['Generated By:', masked_generated_by],
            ...
            if logs:
                ...
                for log in logs:
                    ...
                    row = [
                        ...
                        truncate(ComplianceService.mask_email_for_display(log.get('actor_email', '')), COL_LIMITS[1]),
                        truncate(log.get('action', ''), COL_LIMITS[2]),
                        truncate(log.get('resource_type', ''), COL_LIMITS[3]),
                        truncate(log.get('result', ''), COL_LIMITS[4]),
                        truncate(log.get('severity', ''), COL_LIMITS[5]),
                    ]
                    log_data.append(row)

                logs_table = Table(
                    log_data,
                    colWidths=[1.6*inch, 1.3*inch, 1.6*inch, 1.0*inch, 0.7*inch, 0.7*inch]
                )
